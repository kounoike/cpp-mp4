...
Styp::Styp() {
  m_type = box_type_styp();
}

Styp::Styp(const StypParameters& params)
...
std::string Styp::toStringOnlyData() const {
...
std::uint64_t Styp::writeData(std::ostream& os) const {
...
std::uint64_t Styp::getDataSize() const {
  return 8 + 4 * std::size(m_compatible_brands);
}

std::uint64_t Styp::readData(std::istream& is) {
